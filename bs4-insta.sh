...
#####3#3#
check(){
if wget --spider https://raw.githubusercontent.com/iraq-hacker/hack-instagram/master/update.v1.3 2>/dev/null; then
...
check
option(){
if [ -f "$HOME/hack-instagram/core/update.txt" ];then
cd $HOME/hack-instagram/core
bash update.sh
else
echo " "
...
option
check1(){
if wget --spider https://raw.githubusercontent.com/iraq-hacker/patchupdateznh/main/hack-instagram.v.1 2>/dev/null; then
cd $HOME/hack-instagram/core
echo "igg" > patch.txt
else
echo
...
check1
option1(){
if [ -f "$HOME/hack-instagram/core/patch.txt" ];then
cd $HOME/hack-instagram/core
bash patch.sh
else
echo " "
...
then
echo#do nothing just juck
else
echo -e "\e[91mTOR\e[96m IS NOT RUNNING, PLEASE START TOR IN A NEW SESSION
          BY TYPING\e[92m tor\e[96m AND START hack-instagram AGAIN\e[0m "
exit 1
fi
echo '
...
printf "\n"
printf "  \e[100m\e[1;77m >>  Youtube Channel : \e[1;96m راشد كريم  \e[100m\e[1;77m << \e[0m\n"
printf "\n"
echo 
...
am start -a android.intent.action.VIEW -d https://youtube.com/c/راشدكريم
elif [[ $option == 6 || $option == 06 ]]; then
am start -a android.intent.action.VIEW -d https://t.me/iiwiw
elif [[ $option == 7 || $option == 07 ]]; then
am start -a android.intent.action.VIEW -d https://t.me/Professional_school
elif [[ $option == 8 ]]; then
exit 1
else
echo
